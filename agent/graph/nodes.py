...
# Import Deep Research integration
try:
    from ..integrations.deepresearch_client import (
        DeepResearchClient,
        DeepResearchTimeoutError,
        DeepResearchError
    )
    from ..integrations.research_converter import convert_research_to_facts, validate_research_result
    from ..integrations.research_config import get_research_config
    HAVE_DEEP_RESEARCH = True
except ImportError:
    HAVE_DEEP_RESEARCH = False
    logger.warning("Deep Research integration not available")
...
def _get_llm(temperature: float = 0) -> ChatOpenAI:
    ...
    model = (
        ...
        or "gpt-5"  # ✅ CORRIGIDO: fallback seguro
    )
...
def retrieve_facts(state: AgendaState) -> AgendaState:
    ...
    try:
        ...
        # DEEP RESEARCH INTEGRATION (Strategy 4)
        # If we still have few facts, use Deep Research for comprehensive analysis
        deepresearch_facts = []
        if len(all_facts) < 8 and HAVE_DEEP_RESEARCH and subject_or_query:
            logger.info(f"🔬 Few facts ({len(all_facts)}) - triggering Deep Research for comprehensive analysis...")
            _update_progress(state, "retrieve_facts", "running", "Executing deep research...")
            
            try:
                # Initialize Deep Research client
                deepresearch_client = DeepResearchClient()
                
                # Check health before making request
                health_ok = deepresearch_client.health_check()
                if not health_ok:
                    logger.warning("⚠️ Deep Research health check failed - skipping deep research")
                else:
                    # Build research query from subject and intent
                    intent = state.get("intent", "alignment")
                    language = state.get("language", "pt-BR")
                    
                    research_query = subject_or_query
                    if intent == "alignment":
                        research_query = f"{subject_or_query} - análise detalhada para alinhamento de equipe"
                    elif intent == "decision":
                        research_query = f"{subject_or_query} - pesquisa profunda para tomada de decisão"
                    elif intent == "status":
                        research_query = f"{subject_or_query} - status atual e atualizações recentes"
                    
                    logger.info(f"🔍 Deep Research query: '{research_query[:100]}...'")
                    
                    # 🆕 SMART CONFIG: Get optimal configuration
                    decision = get_research_config(
                        topic=subject_or_query,
                        intent=intent,
                        internal_facts_count=len(all_facts),
                        time_budget=240  # Azure timeout limit
                    )
                    
                    if not decision["should_use"]:
                        logger.info(f"ℹ️  Skipping Deep Research: {decision['reason']}")
                        research_result = None
                    else:
                        config = decision["config"]
                        logger.info(
                            f"🎯 Deep Research config: {config['reasoning']} | "
                            f"Timeout: {config['timeout']}s"
                        )
                    
                        try:
                            # Execute research with optimal config
                            research_result = deepresearch_client.research_sync(
                        ...
                        except (DeepResearchTimeoutError, DeepResearchError) as e:
                            # 🆕 FALLBACK: Tentar novamente com menos steps
                            fallback_steps = config["fallback_steps"]
                            logger.warning(
                                f"⚠️ Deep Research error ({type(e).__name__}) - "
                                f"tentando fallback com {fallback_steps} steps"
                            )
                            try:
                                research_result = deepresearch_client.research_sync(
                                    topic=research_query,
                                ...
                                logger.info(f"✅ Fallback bem-sucedido com {fallback_steps} steps")
                            except Exception as fallback_error:
                                logger.error(f"❌ Deep Research fallback também falhou: {fallback_error}")
                                research_result = None
                    ...
                    # Validate result
                    if research_result and validate_research_result(research_result):
                        logger.info(
                            f"✅ Deep Research completed: "
                            f"{research_result.get('steps_completed')} steps, "
                            f"quality {research_result.get('avg_quality'):.1f}/10"
                        )
                        
                        # Convert research to Meeting Agent facts
                        deepresearch_facts = convert_research_to_facts(
                        ...
                        if deepresearch_facts:
                            logger.info(f"📚 Converted to {len(deepresearch_facts)} Deep Research facts")
                            
                            # Add Deep Research facts to all_facts with priority boost
                            # Apply 1.2x weight multiplier to prioritize deep research
                            for fact in deepresearch_facts:
                                fact["_deepresearch_priority"] = True
                                fact["confidence"] = min(fact.get("confidence", 0.8) * 1.2, 1.0)
                            
                            all_facts.extend(deepresearch_facts)
                            
                            # Optional: Persist Deep Research facts to MongoDB
                            try:
                                for fact in deepresearch_facts:
                                    db.upsert_fact(
                                        org_id=org_id,
                                        ...
                                        source=fact.get("source", "deep_research_agent"),
                                        status=fact.get("status", "published"),
                                        created_at=fact.get("created_at", datetime.utcnow().isoformat()),
                                        metadata=fact.get("metadata", {})
                                    )
                                logger.info(f"💾 Persisted {len(deepresearch_facts)} Deep Research facts to MongoDB")
                            except Exception as persist_error:
                                logger.warning(f"Failed to persist Deep Research facts: {persist_error}")
                                # Continue even if persistence fails
                        else:
                            logger.warning("⚠️ Deep Research returned no facts after conversion")
                    else:
                        logger.warning("⚠️ Deep Research result validation failed")
                    
            except Exception as e:
                logger.error(f"❌ Deep Research error: {str(e)}", exc_info=True)
                # Continue with available facts (graceful degradation)
